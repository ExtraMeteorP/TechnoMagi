...
public class AbilityCastTiltBlock extends AbilityCast
{

    public final static int maxDuration = 1;

    public AbilityCastTiltBlock(String name, ResourceLocation icon)
    {
        super(name, icon);
    }
    ...
    public void use(PlayerTechnomagi technomage, AbilityPayload payload, int abilityMode)
    {
        ...
        if (block instanceof BlockTilted) {
            TileTilted shifted = (TileTilted) world.getTileEntity(payload.blockX, payload.blockY, payload.blockZ);
            ...
            if (item != null) {
                ...
                world.setBlock(payload.blockX, payload.blockY, payload.blockZ, Blocks.tilted);

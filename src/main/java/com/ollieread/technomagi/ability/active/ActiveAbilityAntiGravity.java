...
public class ActiveAbilityAntiGravity extends AbilityActive
{
    protected int cost;

    public ActiveAbilityAntiGravity(String name)
    {
    ...
    {
        if (!charon.player.isPotionActive(PotionTM.antiGravity)) {
            if (decreaseNanites(charon, cost)) {
                if (!charon.player.worldObj.isRemote) {
                    Random rand = new Random();

                    charon.player.addPotionEffect(new PotionEffect(PotionTM.antiGravity.id, 400, 0, false));
                }

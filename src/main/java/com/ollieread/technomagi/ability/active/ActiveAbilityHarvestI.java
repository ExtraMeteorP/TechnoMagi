...
public class ActiveAbilityHarvestI extends AbilityActive
{

    public ActiveAbilityHarvestI(String name)
    {
    ...
    public boolean use(ExtendedPlayerKnowledge charon, Event event)
    {
        if (event instanceof PlayerInteractEvent) {
            ...
            if (interact.action.equals(Action.RIGHT_CLICK_BLOCK)) {
                ...
                if (block != null && block.getHarvestLevel(meta) <= 1 && (block.getHarvestTool(meta) == null || block.getHarvestTool(meta).equals("pickaxe") || block.getHarvestTool(meta).equals("shovel"))) {

...
public class TileEntityAirLight extends TileEntityBasic implements ITileEntityHasMaster
{
    ...
    public boolean validateSelf()
    {
        if (master.setup) {
            if (master.masterX == 0 && master.masterY == 0 && master.masterZ == 0) {
                ...
                int metadata = worldObj.getBlockMetadata(master.masterX, master.masterY, master.masterZ);
                if (!block.equals(Blocks.blockMachine) && metadata == 8) {
                    master.setSetup(false);

                    return false;
                } else {
                    TileEntityMachineAreaLight tile = (TileEntityMachineAreaLight) worldObj.getTileEntity(master.masterX, master.masterY, master.masterZ);
            ...
            for (int j = zStart; j < zEnd; j++) {
                for (int k = yStart; k < yEnd; k++) {
                    if (isValidPosition(worldObj, this, i, k, j)) {
                        ...
                        TileEntityAirLight light = (TileEntityAirLight) worldObj.getTileEntity(i, k, j);
    ...
    public static boolean isValidPosition(World world, TileEntityAirLight tile, int x, int y, int z)
    {
        if (tile.isSetup() && !world.canBlockSeeTheSky(x, y, z) && world.getBlockLightValue(x, y, z) < 14) {
            ...
            if (block.equals(Blocks.blockLightAir)) {
                if (((BlockAirLight) block).validateSelf(world, x, y, z)) {
                    return false;
                }

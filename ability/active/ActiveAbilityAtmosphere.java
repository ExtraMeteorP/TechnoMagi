            ...
            if (!charon.player.isPotionActive(Potion.waterBreathing)) {
                if (decreaseNanites(charon, cost)) {
                    if (!charon.player.worldObj.isRemote) {
                        ...
                        charon.player.worldObj.playSoundEffect((double) charon.player.posX + 0.5D, (double) charon.player.posY + 0.5D, (double) charon.player.posZ + 0.5D, Reference.MODID.toLowerCase() + ":cast", 1.0F, rand.nextFloat() * 0.4F + 0.8F);
